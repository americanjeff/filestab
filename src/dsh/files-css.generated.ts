...
/*The head row doubles as the ref surface: at rest it shows the file's path,
a click/selection on source text morphs it into the exact ref token. The
action cluster hugs the path/token (flex:0 1 auto — natural width, shrinking
with an ellipsis only when it runs out of room): buttons at the row's far
end would read as "acts on the file", not "acts on the ref". One line, fixed
height (never wraps); user-select off so a drag from the row into the file
text can't build a mixed-anchor selection the resolvers reject, and so
clicking the buttons never collapses the live selection.*/
.dswFiles_paneHead{display:flex;align-items:center;gap:6px;padding:6px 10px;border-bottom:1px solid var(--dsw-alias-border-l3);user-select:none;-webkit-user-select:none;min-height:30px}
.dswFiles_paneHeadPath{flex:0 1 auto;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:13px;font-weight:600}
.dswFiles_paneHeadToken{flex:0 1 auto;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;font-size:12px;line-height:18px;color:var(--dsw-alias-label-primary)}
.dswFiles_paneHeadBtns{display:flex;gap:2px;align-items:center;flex:none}
.dswFiles_headBtn{cursor:pointer;background:0 0;border:none;border-radius:5px;padding:1px 5px;font-size:11px;line-height:18px;color:var(--dsw-alias-label-secondary);display:inline-flex;align-items:center;gap:3px;white-space:nowrap}
.dswFiles_headBtn:hover{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-interactive-bg-hover)}
